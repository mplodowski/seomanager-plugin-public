...
categories), that record is the source of the SEO tags. A field the record leaves blank falls back to the same field
...
render would really use instead: the title prefix and suffix from the settings around an empty **SEO Title**, the
**SEO Title** in place of an empty **OG Title**, the **SEO Description** in place of an empty **OG Description**,
...
- **Every site of a group needs its own language.** A site with no locale of its own reports the application locale, so
  ...
  names the language responsible.
...
- one sitemap per site in multisite, with translated URLs and `xhtml:link` hreflang alternates for every entry, post,
  ...
  points at the translated URL.

Alternates cover the same sites as the `seoTags` hreflang tags — see the two rules under **Hreflang Tags**.
...
Then add the SEO columns to the model's table. This command scans all models implementing the behavior:
...
**Diagnostics** tab names every table this happened to and adds the columns to all of them on a button, so a fresh clone
or a new production environment does not stay silently without SEO.
...
has been deleted from the media library describes the site with a broken identity, and a `robots.txt` that never names
the sitemap leaves crawlers to find it on their own.
